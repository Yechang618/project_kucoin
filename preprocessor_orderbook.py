...
def generate_df_one_day(symbols, date = None, dataset_name = 'book_snapshot_5_'):
    df = None
    address = 'datasets'
...
def generate_df(symbols, start_date = None, end_date = None, dataset_name = None):
    df = None
    date_range = pd.date_range(start=start_date, end=end_date)
    for date in date_range:
        date_str = date.strftime('%Y-%m-%d')
        print(f"Processing date: {date_str}")
        df_day = generate_df_one_day(symbols, date_str, dataset_name)
        if df is None:
            df = df_day.copy()
        else:
...
if __name__ == "__main__":
    ...
    df = generate_df(symbols, start_date="2025-09-01", end_date="2025-09-20", dataset_name=dataset_name)
    # df = generate_df(symbols, dataset_name)
    print(df.head(5))
    print(df.info())
    df.to_csv('basis_' + dataset_name + f"_{symbols[0]}" +'_3w.csv', index=True, header=True, quoting=csv.QUOTE_NONNUMERIC)
    print("DataFrame saved to 'basis_" + dataset_name + f"_{symbols[0]}_3w" + ".csv'  successfully.")
