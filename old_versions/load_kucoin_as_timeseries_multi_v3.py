...
def process_symbol(symbol, csv_dir="kucoin_csv", output_dir="kucoin_series"):
    ...
    try:
        ...
        # 8. 【核心逻辑】用区间平均值填充 NaN
        # 先获取所有数值列（排除 timestamp 和 funding_rate）
        numeric_cols = [col for col in df_clean.columns if col not in ['timestamp', 'funding_rate']]
        ...
        for idx, row in funding_events.iterrows():
            # 从上一个 funding 事件到当前事件（包括当前）
            if prev_time is None:
                # 第一个事件：使用从数据开始到当前
                interval_data = df_clean.loc[:idx, numeric_cols]
            else:
                # 后续事件：使用 (prev_time, current_time] 区间
                interval_data = df_clean.loc[prev_time:idx, numeric_cols]
            ...
            for col in numeric_cols:
                if pd.isna(new_row[col]):
                    ...
                    if not col_data.empty:
                        new_row[col] = col_data.mean()
                    # 如果区间内全为 NaN，保留 NaN（后续过滤）
            
            result_records.append(new_row)
            prev_time = idx

        # 转为 DataFrame
        df_result = pd.DataFrame(result_records, index=funding_events.index)

        # 9. 移除仍包含 NaN 的行
        df_final = df_result.dropna(how='any')
        if df_final.empty:
            print(f"⚠️  No complete funding events after alignment for {symbol}. Skipping.")
            return False

        # 10. 保存结果
        output_path = Path(output_dir) / f"{symbol}_funding_avg{version}.csv"
